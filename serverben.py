...
def responsesEntered():
        questions = [{"a":0,"b":0,"c":0,"d":0,"e":0} for i in xrange(40)]
...
def enter():
        ...
        if request.method == "POST":
                ...
                if data is None:
                        response = ""
						",".join(request.form['Q' + str(i)] for i in xrange(1, 41))
						# string.Join(", ", from idx in Enumerable.Range(1, 41) select request.Form["Q" + idx.ToString());
						# string.Join(",", Enumerable.Range(1, 41).Select(idx => request.form["Q" + idx.ToString()))
